...
public interface OrderRepository extends CrudRepository<Order, String> {
    ...
    @Query("SELECT * FROM book_order join region using(address_street_postal) WHERE order_id = :order_id")
    Order findByOrderId(@Param("order_id") int order_id);

    @Query("SELECT * FROM book_order join region using(address_street_postal) WHERE shipping_id = :shipping_id")
    Order findByShippingId(@Param("shipping_id") String shipping_id);

    @Query("SELECT * FROM book_order join region using(address_street_postal)")
    List<Order> findAllOrders();

    @Query("SELECT * FROM book_order join region using(address_street_postal) WHERE email = :email")
    ...
    @Query("INSERT INTO book_order(email, date, shipping_id, address_street_num, address_street_name, address_street_postal) values(:email, :date, :shipping_id, :street_num, :street_name, :postal) returning order_id")
    long addOrder(@Param("email") String email, @Param("date") Timestamp date, @Param("shipping_id") String shipping_id, @Param("street_num") String street_num, @Param("street_name") String street_name, @Param("postal") String postal);
